#!/usr/bin/env python3
# parse.py - Parse Open-Meteo API response into clean dicts
...
def degrees_to_dir(deg):
    """Convert wind degrees to cardinal + arrow. Safe for None/0."""
    if deg is None:
        return "---"
    try:
        deg = float(deg) % 360
...
def parse_current(data_str: str) -> dict:
    """Parse current conditions block."""
